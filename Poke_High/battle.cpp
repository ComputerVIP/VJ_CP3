...
void score_win(Fl_Widget* w, int scr, Fl_Window* main_win, Fl_Input* nameInput, Fl_Window* score) {
    fl_message("You win the battle!");
    score ->show();
    nameInput->callback([](Fl_Window score, Fl_input nameInput) {
        int scr = *(int*)data;
        ifstream infile;
        ...
        if (infile.is_open()) {
            ...
            while (getline(infile, line)) {
                ...
                entry.name = name;
                getline(ss, wins, ',');
                entry.wins = stoi(wins);
                getline(ss, losses, ',');
                entry.losses = stoi(losses);
                scores.push_back(entry);
            }
        }
        std::string name = static_cast<Fl_Input*>(w)->value();
        cout << "Name entered: " << name << endl;
        fl_message("Score recorded for %s with %d wins!", name.c_str(), scr);
        score->hide();
    }, score);
}
...
void lightAttackCallback(Fl_Widget* w, void* data) {
    ...
    if (opponentStats.health <= 0) {
        ...
        fl_message("You win!");
        battle_win->hide();
        score_win(w, 1, main_win, static_cast<Fl_Input*>(nullptr), static_cast<Fl_Window*>(nullptr));
        main_win->show();
        return;
    }
...
"You are battling %s! Your type is %s and theirs is %s", opponent.name.c_str(), player.type.c_str(), opponent.type.c_str());
    ...
    Fl_Window* score = new Fl_Window(200, 100, "Score");
    Fl_Input* nameInput = new Fl_Input(50, 20, 100, 30, "Enter your name:");
    Fl_Button* submitBtn = new Fl_Button(60, 60, 80, 30, "Submit");
    score ->end();
    score ->hide();
